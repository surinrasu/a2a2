...
use super::error::{BluetoothError, Result};
...
fn capture_thread(
    ...
    // Configure hardware parameters
    {
        let hwp = HwParams::any(&pcm)
            .map_err(|e| BluetoothError::Alsa(format!("Failed to get hardware params: {}", e)))?;

        hwp.set_channels(config.channels)
            .map_err(|e| BluetoothError::Alsa(format!("Failed to set channels: {}", e)))?;

        hwp.set_rate(config.sample_rate, ValueOr::Nearest)
            .map_err(|e| BluetoothError::Alsa(format!("Failed to set sample rate: {}", e)))?;
        ...
        hwp.set_format(alsa_format)
            .map_err(|e| BluetoothError::Alsa(format!("Failed to set format: {}", e)))?;

        hwp.set_access(Access::RWInterleaved)
            .map_err(|e| BluetoothError::Alsa(format!("Failed to set access: {}", e)))?;

        hwp.set_buffer_size(BUFFER_FRAMES as alsa::pcm::Frames)
            .map_err(|e| BluetoothError::Alsa(format!("Failed to set buffer size: {}", e)))?;
        ...
        pcm.hw_params(&hwp)
            .map_err(|e| BluetoothError::Alsa(format!("Failed to apply hardware params: {}", e)))?;
    }

    // Prepare PCM for use
    pcm.prepare()
        .map_err(|e| BluetoothError::Alsa(format!("Failed to prepare PCM: {}", e)))?;
...
fn capture_s16(
    ...
    let io = pcm
        .io_i16()
        .map_err(|e| BluetoothError::Alsa(format!("Failed to get I/O interface: {}", e)))?;
...
fn capture_s24_with_resample(
    ...
    // Uses the shared AirPlay resampler with mastering-quality settings (sinc_len: 512)
    let mut resampler = liba2::audio::resampler::Resampler::with_chunk_size(
        SAMPLE_RATE_HD,
        DEFAULT_SAMPLE_RATE,
        ...
        match io.readi(&mut raw_buffer[..bytes_to_read]) {
            Ok(frames_read) => {
                ...
                if warmup_frames_remaining > 0 {
                    ...
                    if warmup_frames_remaining == 0 {
                        info!(
                            "Warmup complete - discarded {} initial ALSA frames, now streaming",
                            WARMUP_FRAMES_TO_DISCARD
                        );
                    } else {
                        debug!(
                            "Warmup: discarding frame ({} remaining)",
                ...
                let samples = liba2::audio::resampler::interleave_with_dither(&resampled);

                let frame = CapturedFrame { samples, timestamp };
